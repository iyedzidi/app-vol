...
void statistique::makePolt()
{
    ...
    regen->setName("Moyenne des voyageurs");
    ...
    QSqlQuery query1("select AVG(NB_VOYAGEURS) from VOOL where destination ='tunisie'");
    while (query1.next()) {
        int  nbr_faute;
                           int  nbr_fautee = query1.value(0).toInt();
                           QSqlQuery query2("select AVG(NB_VOYAGEURS) from VOOL where destination !='tunisie'");
                           while (query2.next()) {
